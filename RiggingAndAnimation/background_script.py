...
def render(render_path):
    ...
    bpy.context.scene.render.filepath  = cf.MAIN_DIR + "\\test\\"
...
def exportmodel(export_path):
    bpy.ops.export_scene.fbx(export_path, type='INVOKE_DEFAULT')
...
def main():
    ...
    cf.setupSun()
    cf.setupCamera(bpy.context.scene)
    import_model()
    add_armature()
    recenter()
    add_texture()
    animate()
    render(cf.MAIN_DIR + cf.RENDER_DIR)
    exportmodel(cf.MAIN_DIR + cf.EXPORT_PATH)
